...
std::string Utils::TrimStr(const std::string& str) {
  std::string ret = str;
...
void Utils::SplitByDelimiter(const std::string& str,
  const std::string& delimiter, std::vector<std::string>* vec_str) {
  boost::split(*vec_str, str, boost::is_any_of(delimiter));
}
...
void Utils::PrintHex(const uint8_t* ctx, const int& size, std::ostream& os) {
  std::ostringstream oss;
